...
abi_uri() {
	local baseuri="https://download.oracle.com/graalvm/$(ver_cut 1)/archive"
	local musl=
	local os=linux
	...
	echo "${2-$1}? (
		${musl:+ elibc_musl? ( }
			${baseuri}/graalvm-jdk-${PVH}_linux-x64_bin.tar.gz
		${musl:+ ) } )"
}
...
SRC_URI="
	$(abi_uri x64 amd64)
"
S="${WORKDIR}/graalvm-jdk-${MY_PV}+${FIXED_SUFFIX}"
